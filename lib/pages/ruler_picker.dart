...
class RulerPickerState extends State<RulerPicker> {
            ...
            child: NotificationListener(
              onNotification: (scrollNotification) {
                if (scrollNotification is ScrollStartNotification) {
                  ...
                  if (!isPosFixed) {
                    isPosFixed = true;
                    fixPosition((scrollNotification.metrics.pixels / 1)
                            .roundToDouble() *
                        1);
  ...
  void initState() {
    ...
    scrollController.addListener(() {
      setState(() {
        widget._value = double.parse((scrollController.offset / 5)
            .toStringAsFixed(widget.fractionDigits));
        if (widget._value! < 0)
          scrollController = ScrollController(
              initialScrollOffset: widget.controller.value.toDouble());
        ;
